...
O cron `/api/cron/release-expired-orders` usa `CRON_SECRET`. Configure domínio HTTPS, PagBank sandbox, Cloudinary e Google OAuth com callbacks do domínio. Use Deployments para logs/rollback; nunca faça rollback de banco sem plano de migration e backup.
